"""Admin-only views - DynamoDB implementation - Async Version"""
...
from api.dynamodb.async_repositories import (
    AsyncUserRepository,
...
class UserManagementView(APIView):
    """User management view for admins - DynamoDB implementation - Async Version"""
    ...
    async def get(self, request):
        ...
        try:
            ...
            async with AsyncDynamoDBClient.get_resource() as resource:
                ...
                users_data = []
                for user in users:
                    # Get subscription plan details - async
                    plan_data = None
                    if user.get('subscription_plan_id'):
                        plan = await plan_repo.get_plan(user['subscription_plan_id'])
                        if plan:
                            plan_data = {
                                'id': plan['id'],
                                'name': plan['name'],
    ...
    async def patch(self, request, user_id=None):
        ...
        try:
            ...
            async with AsyncDynamoDBClient.get_resource() as resource:
                ...
                if plan_id:
                    # Verify plan exists - async
                    plan = await plan_repo.get_plan(int(plan_id))
                    if not plan:
                        return Response(
                    ...
                    # Get updated user with plan details - async
                    user = await user_repo.get_user_by_id(int(user_id))
                    plan_data = {
                        'id': plan['id'],
                        'name': plan['name'],
    ...
    async def put(self, request, user_id=None):
        ...
        return await self.patch(request, user_id)


class SubscriptionPlanManagementView(APIView):
    """Subscription plan management view for admins - DynamoDB implementation - Async Version"""
    permission_classes = [IsAuthenticated]

    async def get(self, request, plan_id=None):
    ...
    async def post(self, request):
        ...
        try:
            ...
            async with AsyncDynamoDBClient.get_resource() as resource:
                ...
                # Generate new plan ID (get max ID + 1) - async
                plans = await plan_repo.list_plans()
                ...
                # Create plan - async
                await plan_repo.create_plan(plan_data)
    ...
    async def patch(self, request, plan_id):
        ...
        try:
            ...
            async with AsyncDynamoDBClient.get_resource() as resource:
                ...
                # Update plan - async
                updated_plan = await plan_repo.update_plan(int(plan_id), updates)
    ...
    async def delete(self, request, plan_id):
        ...
        try:
            ...
            async with AsyncDynamoDBClient.get_resource() as resource:
                ...
                # Delete plan - async
                await plan_repo.delete_plan(int(plan_id))
...
class UsageStatsView(APIView):
    """Usage statistics view for admins - DynamoDB implementation with caching - Async Version"""
    permission_classes = [IsAuthenticated]
    ...
    async def get(self, request):
        ...
        # Try to get from cache (15 minute TTL) - async
        ...
        try:
            ...
            async with AsyncDynamoDBClient.get_resource() as resource:
                ...
                problems, _ = await problem_repo.list_completed_problems(limit=10000)
                total_problems = len(problems)

            # Usage stats for the period (using date range) (outside async context)
            ...
            # Aggregate usage across all users - async
            async with AsyncDynamoDBClient.get_resource() as resource:
                table = await resource.Table(AsyncDynamoDBClient._table_name)
                usage_repo = AsyncUsageLogRepository(table)
                ...
                for user in active_users:
                    ...
                    # Get usage logs for this user in the date range - async
                    user_logs = await usage_repo.list_user_usage(
                        user_id=user_id,
            ...
            # Get plan names for top users - async
            top_users_data = []
            async with AsyncDynamoDBClient.get_resource() as resource:
                table = await resource.Table(AsyncDynamoDBClient._table_name)
                plan_repo = AsyncSubscriptionPlanRepository(table)
            ...
            # Cache the result for 15 minutes (900 seconds) - async
            await sync_to_async(cache.set)(cache_key, response_data, 900)
...
class ProblemReviewView(APIView):
    """Problem review management for admins - DynamoDB implementation - Async Version"""
    permission_classes = [IsAuthenticated]
    ...
    async def get(self, request):
        ...
        try:
            ...
            async with AsyncDynamoDBClient.get_resource() as resource:
                ...
                # Get all problems (both drafts and completed) - async
                all_problems = []
                drafts, _ = await problem_repo.list_draft_problems(limit=1000)
                completed, _ = await problem_repo.list_completed_problems(limit=1000)
                all_problems.extend(drafts)
                all_problems.extend(completed)

            # Filter by review status (outside async context)
            ...
            # Transform to response format with test case count - async
            problems_data = []
            async with AsyncDynamoDBClient.get_resource() as resource:
                table = await resource.Table(AsyncDynamoDBClient._table_name)
                problem_repo = AsyncProblemRepository(table)
                ...
                for problem in all_problems:
                    # Get test cases count - async
                    testcases = await problem_repo.get_testcases(
    ...
    async def patch(self, request, problem_id=None, platform=None, problem_identifier=None):
        ...
        try:
            ...
            async with AsyncDynamoDBClient.get_resource() as resource:
                ...
                # Get problem - async
                problem = await problem_repo.get_problem(
                    platform=platform,
                ...
                updated_problem = await problem_repo.update_problem(
                    platform=platform,
                    problem_id=problem_identifier,
                    updates=updates
                )
    ...
    async def put(self, request, problem_id=None, platform=None, problem_identifier=None):
        """Alias for patch"""
        return await self.patch(request, problem_id, platform, problem_identifier)
