...
cat > /etc/algoitny-worker.env <<EOF
DEBUG=False
...
chmod 600 /etc/algoitny-worker.env

# Create systemd service file
cat > /etc/systemd/system/algoitny-worker.service <<'EOF'
[Unit]
Description=AlgoItny Celery Worker
...
TimeoutStartSec=300
User=root
EnvironmentFile=/etc/algoitny-worker.env
ExecStartPre=-/usr/bin/docker stop algoitny-worker
ExecStartPre=-/usr/bin/docker rm algoitny-worker
ExecStart=/usr/bin/docker run --rm \
  --name algoitny-worker \
  -v /var/log/algoitny:/app/logs \
  -v /app/tmp:/app/tmp \
  -v /app/config/config.yaml:/app/config/config.yaml:ro \
  --env-file /etc/algoitny-worker.env \
  ${ecr_image_url} \
  bash /app/scripts/start_celery.sh
ExecStop=/usr/bin/docker stop algoitny-worker
...
echo "Starting AlgoItny Worker service..."
systemctl daemon-reload
systemctl enable algoitny-worker.service
systemctl start algoitny-worker.service
...
for i in {1..10}; do
  if systemctl is-active --quiet algoitny-worker.service; then
...
systemctl status algoitny-worker.service --no-pager
