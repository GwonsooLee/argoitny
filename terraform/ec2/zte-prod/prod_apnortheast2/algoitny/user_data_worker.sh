        ...
        "collect_list": [
          {
            ...
            "log_group_name": "/aws/ec2/algoitny-worker-${env_suffix}",
...
# Get DynamoDB and ElastiCache endpoints
...
# Run Docker container for Celery Worker
...
docker logs -f algoitny-worker >> /var/log/docker.log 2>&1 &
...
for i in {1..10}; do
  if docker ps | grep -q algoitny-worker; then
