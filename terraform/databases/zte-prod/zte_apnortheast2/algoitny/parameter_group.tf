...
resource "aws_db_parameter_group" "algoitny_aurora_pg" {
  name = "algoitny-aurora-${data.terraform_remote_state.vpc.outputs.shard_id}-pg"
...
resource "aws_rds_cluster_parameter_group" "algoitny_aurora_cluster_pg" {
  name = "algoitny-aurora-${data.terraform_remote_state.vpc.outputs.shard_id}-cluster-pg"

  # Please change this value to version you want to use 
  family = "aurora-mysql5.7"

  description = "algoitny RDS cluster parameter group"
